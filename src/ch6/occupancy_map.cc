...
//note: 注意这里的模板应该是在图像（像素）坐标系下，这样才能对应后面的角度计算
...
/// @brief 输出模板中磨某一角度在真实scan中对应夹角的range
/// @param angle 
/// @param scan 
/// @return 
double OccupancyMap::FindRangeInAngle(double angle, Scan2d::Ptr scan) {
    ...
    // note: 利用int()圆整并不是想象的四舍五入，经测试int(2.3) int(2.9)均输出2
    int angle_index = int((angle - scan->angle_min) / scan->angle_increment);
    if (angle_index < 0 || angle_index >= scan->ranges.size()) {
        return 0.0;
    }
    
    // 这个angle_index_p的作用是什么？？？--> 对应解答了为什么这边有个+1索引操作，当所计算夹角处于两跟射线中间时，对两根线处理，看看哪一个更符合实际
    ...
    if (angle_index_p >= scan->ranges.size()) {
        ...
        // 插值
        // 计算未圆整index与圆整index的差值，当两个index对应直线偏差较大时，s大于0.5时也就表明这根直线更贴合+1后index
        ...
        if (range2 < scan->range_min || range2 > scan->range_max) {
            // 有个疑问，range2不可以，那又怎么判断range1可行呢？
            range = range1;
