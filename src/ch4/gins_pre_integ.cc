...
void GinsPreInteg::SetOptions(sad::GinsPreInteg::Options options) {
    ...
    // 6*6维，从矩阵中第9行第9列开始
    prior_info_.block<6, 6>(9, 9) = Mat6d ::Identity() * 1e6; //也就是说这里将ba、bg这些随机游走的协方差设的更小啦
    
    if (this_frame_) {
        // 这里设定的ba bg初始值在初始化和优化后都会被赋值
...
void GinsPreInteg::AddOdom(const sad::Odom& odom) {
    ...
    // sk changed 针对课后习题6（在基于预积分的GINS系统中，考虑如何防止因为长时间没有RTK观测而发现位移状态发散的情况，实现通过Odom触发优化的方法）
    // 积分到GNSS时刻
...
void GinsPreInteg::Optimize() {
    // sk changed: 由于odom gnss同时触发，所以imu预积分时间间隔可能会比较短，所以需要更改时间增量值
...
void GinsPreInteg::Optimize_NGnss(){
    // sk changed: 由于odom gnss同时触发，所以imu预积分时间间隔可能会比较短，所以需要更改时间增量值
    if (pre_integ_->dt_ < 1e-2) {
        // 未得到积分
        return;
